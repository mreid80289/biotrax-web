// BioTrax — hi-fi landing page using real app screenshots in iPhone frames.
...
// ─────────────────────────── iPhone frame ───────────────────────────
// Slim sage-green iPhone 15 sleeve drawn in CSS. Crisp at any size.
function IPhone({ src, width = 280, alt, style = {}, shadow = true, tilt = 0, innerStyle = {}, imgStyle = {} }) {
  const aspect = 2796 / 1290; // real iPhone 15 ratio
  const height = width * aspect;
  const radius = width * 0.135;
  const bezel = Math.max(4, width * 0.022); // slimmer — was 0.038
  const screenRadius = radius - bezel * 0.65;
  return (
    <div style={{
      width, height,
      borderRadius: radius,
      // Sage-green titanium bezel with subtle metallic shading
...
// ─────────────────────────── Tilt Phone (interactive 3D) ───────────────────────────
function TiltPhone({ src, width = 360, alt }) {
  const ref = React.useRef(null);
  const [tilt, setTilt] = React.useState({ x: 0, y: 0, mx: 50, my: 50, active: false });
  const idleRef = React.useRef(0);

  // Idle drift — gentle parallax even when the cursor is elsewhere, to invite interaction.
  React.useEffect(() => {
    let raf;
  ...
  const onMove = (e) => {
    ...
    const rx = (0.5 - py) * 22;  // x-axis rotation
    const ry = (px - 0.5) * 26;  // y-axis rotation
    setTilt({ x: rx, y: ry, mx: px * 100, my: py * 100, active: true });
  };
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Floating wellness badge — emerges out of screen for depth */}
        <div style={{
          position: 'absolute',
          left: '-18%', top: '36%',
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
        {['The Coach', 'How It Works', 'The App', 'Investors'].map(l => (
          <a key={l} style={{ fontSize: 13.5, color: C.inkDim, textDecoration: 'none', fontWeight: 450 }}>{l}</a>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <a style={{ fontSize: 13.5, color: C.inkDim, textDecoration: 'none', fontWeight: 450 }}>Sign in</a>
        <button style={{
  ...
  return (
    <img
      ...
      style={{
        ...
        // Crop the wordmark off — keep only the square mark (top ~74% of the file)
        objectFit: 'cover', objectPosition: 'top center',
        aspectRatio: '1 / 1',
      }}
    />
  ...
  return (
    <section style={{
      ...
      <div style={{
        ...
        {/* Hero phone — interactive 3D tilt */}
...
function ScorePlayground() {
  const [sleep, setSleep] = React.useState(76);
  const [stress, setStress] = React.useState(35);
  const [social, setSocial] = React.useState(75);
  const [home, setHome] = React.useState(4);

  // Score logic, mirroring the spec:
  //   Sleep      30 pts — full at 60+, none below 30
  //   Stress     20 pts — INVERTED: full at <=20, none at 80+
  //   Social     30 pts — full at 60+, drops below 40
  //   Home       20 pts — full <=18h, drops fast as it nears 72h
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 36, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{
                ...
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: coachState.dot, boxShadow: `0 0 10px ${coachState.dot}` }} />
                {coachState.text}
              </div>
            </div>
          </div>
  ...
  return (
    <section style={{ padding: '140px 48px', borderTop: `1px solid ${C.border}`, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, position: 'relative' }}>
        ...
        <form style={{ display: 'flex', gap: 8, marginTop: 16, width: '100%', maxWidth: 460 }} onSubmit={(e) => e.preventDefault()}>
          <input
            type="email"
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: 40 }}>
        ...
        {[
          ...
          <div key={col.t} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {col.l.map(x => (
              <a key={x} style={{ fontFamily: fontBody, fontSize: 13.5, color: C.inkDim, textDecoration: 'none' }}>{x}</a>
            ))}
          </div>
        ))}
      </div>
...
function App() {
  ...
  const headline = HEADLINES[tweaks.headlineSet] || HEADLINES.warm;
  return (
